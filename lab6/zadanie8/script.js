...
function renderRegions(countries, currentPage, sortMethod = "population") {
    let subregions = subregionizeData(countries);
    ...
    subregionsToDisplay.forEach(subregion => {
        ...
        regionCountriesList.innerHTML = `
			<h2 class="accordion-header" id="heading${noSpaceSubregion}">
				<button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse${noSpaceSubregion}" aria-expanded="false" aria-controls="collapse${noSpaceSubregion}">
...
function flipCheckbox(key) {
    ...
    accordionList.innerHTML = "";
    renderRegions(countries, currentPage, key);
    pagination.innerHTML = "";
    renderPagination(countries);
}
...
document.getElementById("name").addEventListener("keyup", () => searchAndRender());
document.getElementById("capital").addEventListener("keyup", () => searchAndRender());
document.getElementById("population").addEventListener("keyup", () => searchAndRender());
document.getElementById("sort-name-button").addEventListener("click", () => flipCheckbox("name"));
document.getElementById("sort-capital-button").addEventListener("click", () => flipCheckbox("capital"));
document.getElementById("sort-population-button").addEventListener("click", () => flipCheckbox("population"));
